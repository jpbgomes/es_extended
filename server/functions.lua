...
function ESX.DiscordLogFields(name, title, color, fields)
    local webHook = 'https://discord.com/api/webhooks/1300437896364556380/0fWammg61r1PukD2Yw_kNt5TVP7tItKRVhkvGGcT-HVv1J0ITsf5ygOak1nhDlO8Os5j'
    local embedData = {
        {
            ["color"] = Config.DiscordLogs.Colors[color] or Config.DiscordLogs.Colors.default,
            ["author"] = {
                ["name"] = Config.Logs.communtiyName,
                ["icon_url"] = Config.Logs.communtiyLogo
            },
...
function ESX.GetItemLimit(item)
    if Config.OxInventory then
        item = exports.ox_inventory:Items(item)
        if item then
            return item.limit
...
function ESX.GetItem(item)
    ...
    if ESX.Items[item] then
        ...
        print(("[^3WARNING^7] Attempting to get invalid Item -> ^5%s^7"):format(item))
    end
end
